...
from src.nova.utils.settings import settings


BASE_SYSTEM_PROMPT = """
You are Nova, a smart laptop assistant with memory.
...
3. GENERAL CHAT: For greetings, casual conversation, jokes, personal statements, or anything that's NOT a command or question requiring research. 
   - Examples: "Hey", "Hi nova", "How are you?", "I'm tired", "That's cool", "Thanks", "goodbye"
   - For chat responses, ALWAYS generate a friendly, conversational response that directly replies to what the user said.

If the request is a QUESTION requiring a search, use "web_research".
If the request is an instruction to OPERATE the computer, use the specific ACTION command.
If the request is ANYTHING ELSE, use "general_chat" with a proper conversational response.
...
21. Chat Response: {"action": "general_chat", "response": "Your friendly conversational response here"}
    (Triggers: "Hey", "Hi", "How are you?", "I'm tired", "Thanks", "goodbye", any casual conversation)
    *** For chat responses, ALWAYS provide a warm, helpful, engaging response that directly addresses what the user said ***
    *** Examples: 
    - User: "Hey nova" -> {"action": "general_chat", "response": "Hey there! I'm Nova, your smart assistant. What can I help you with?"}
    - User: "I'm tired" -> {"action": "general_chat", "response": "I understand, working all day can be exhausting. Need me to help with anything to make your day easier?"}
    - User: "How are you?" -> {"action": "general_chat", "response": "I'm doing great! Ready to help you out. What's on your mind?"}
...
*** FALLBACK RULE ***
If the user's message doesn't clearly match any ACTION command or RESEARCH query, return a CHAT response!
- General chat is NOT a last resort - it's the default for anything that's NOT a system operation or research query.
- When in doubt about whether it's a command, treat it as friendly conversation.
- Always respond warmly and helpfully to casual messages.
"""
...
def process_command(user_input):
    ...
    try:
        ...
        if not content or not content.strip():
            print(f"Error: Empty response from Qwen")
            return [{"action": "general_chat", "response": "I didn't get a valid response. Please try again."}]
        ...
        find_triggers = ["find that", "get that", "get me that", "send that", "give me that", "that file", "that pdf", "that document", "that excel", "that image", "that video", "i was reading", "i opened", "i was working on", "file i", "document i"]
        ...
        print(f"Error: Invalid JSON from Qwen: {e}")
        print(f"Content was: {content[:200] if content else 'empty'}")
        
        # Try to recover from common LLM response patterns
        lower_content = content.lower().strip() if content else ""
        
        if "web_research" in lower_content or "research" in lower_content:
            return [{"action": "web_research", "query": user_input}]
        elif "find" in lower_content or "file" in lower_content:
            return [{"action": "find_file", "query": user_input}]
        elif "screenshot" in lower_content:
            return [{"action": "take_screenshot"}]
        
        return [{"action": "general_chat", "response": "I had trouble understanding the command. Please try again."}]
