"""
File Finder Module for Nova Desktop Assistant
...
def search_files_by_content(search_text: str, folder_path: Optional[str] = None, 
                            max_results: int = 10) -> Dict:
    """
    Search for text content inside files in a specified folder (default: Desktop)
    
    Args:
        search_text: Text to search for inside files
        folder_path: Folder to search in (default: Desktop)
        max_results: Maximum number of files to return
        
    Returns:
        Dictionary with status and matching files
    """
    # Use Desktop folder as default
    ...
    if not os.path.exists(desktop_path):
        return {
            "status": "error",
            "message": f"❌ Folder not found: {desktop_path}"
        }
    
    matching_files = []
    search_text_lower = search_text.lower()
    
    # File extensions to search (expanded to include more types)
    # Text-based files that can contain searchable content
    text_extensions = {
        # Documents
        '.txt', '.md', '.json', '.csv', '.xml', '.html', '.css', '.js', '.py',
        '.java', '.cpp', '.c', '.h', '.php', '.rb', '.go', '.rs', '.sql',
        '.log', '.conf', '.ini', '.yaml', '.yml', '.sh', '.bat', '.ps1',
        '.doc', '.docx', '.pdf', '.rtf', '.odt', '.ods', '.pptx',
        # Config files (no extension)
        '.env', '.gitignore', '.htaccess', 
        # Additional text formats
        '.tex', '.rst', '.adoc', '.asciidoc',
        '.properties', '.gradle', '.maven', '.sbt',
        '.dockerfile', '.makefile', '.cmake',
    }
    
    try:
        # Walk through all files in the folder (just Desktop, not subdirectories for speed)
        files_checked = 0
        for filename in os.listdir(desktop_path):
            ...
            try:
                # Read file content - try UTF-8 first, then with errors='ignore'
                file_content = None
                try:
                    ...
                    print(f"   ❌ Could not read: {str(read_error)[:40]}")
                    continue
                ...
                # Search for text in lines (case-insensitive)
                found_lines = []
                for line_num, line in enumerate(lines, 1):
                    ...
                    if search_text_lower in line.lower():
                        found_lines.append({
                            ...
                            'line_content': line.strip()[:100]  # Truncate to 100 chars
                        })
                        print(f"   ✅ Match found at line {line_num}: {line.strip()[:60]}")
                ...
                if found_lines:
                    ...
                    matching_files.append({
                        ...
                        'matches': found_lines[:3],  # Show first 3 matches
                        'file_type': file_ext.replace('.', '').lower() or 'txt'
                    })
                ...
                print(f"   ⚠️  Error processing: {str(e)[:40]}")
                continue
        ...
        if not matching_files:
            return {
                "status": "not_found",
                "message": f"❌ No files found containing '{search_text}' in Desktop folder"
            }
        
        return {
            "status": "found",
            "message": f"✅ Found {len(matching_files)} file(s) containing '{search_text}'",
            "files": matching_files,
            "total_matches": sum(f['matches_count'] for f in matching_files)
        }
